# OVERSPLIT REMOVAL BUT ADD TOGETHER -------------------------------------------------------
...
rt_tol<-0.05  
oversplit_num<-2 
max_oversplit_num<-5 #maximum repeating number before its "junk"
...
write.csv(data_export, "info_about_features_merged.csv", row.names = FALSE)
write.csv(CIs_to_drop, "deleted_rows.csv", row.names = FALSE)

write.csv(cleaned_data, "OSmerged_quant.csv", row.names = FALSE) #make sure to keep .csv at end
